...
void twi_start(void) {    
    // TODO 0: Send START condition (using TWCR)
    TWCR = 0;
	TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWSTA) /* | (1 << TODO_START_BIT???) */;
...
void twi_write(uint8_t data) {
    ...
    TWDR = data;
	TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWEA) /* | (1 << TODO_ACKNOWLEDGE) */;
